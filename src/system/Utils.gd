...
# 	if node is AudioStreamPlayer or node is AnimatedSprite2D:
# 		if value:
...
# 		if node is Control:
# 			node.connect('gui_input',Callable(self,'input_probe').bind(node.get_path()))
...
# 	return stamp.format(Time.get_datetime_dict_from_system())
