...
# func _ready():
# 	websocket.connect('connection_established',Callable(self,'on_connection_established'))
# 	websocket.connect('data_received',Callable(self,'on_received_data'))
...
# 	var packet: PackedByteArray = websocket.get_peer(1).get_packet()
