...
var server_info = {name = 'Server', max_players = 10, used_port = 9099}
...
func request_ping(dest_id):
	ping_data[dest_id].timer.connect(
		'timeout', self, '_on_ping_timeout', [dest_id], CONNECT_ONESHOT
	)
	ping_data[dest_id].timer.start(ping_timeout)
	rpc_unreliable_id(
		dest_id, 'on_ping', ping_data[dest_id].signature, ping_data[dest_id].last_ping
	)
...
remote func on_pong(signature):
	...
	if ping_data[peer_id].signature == signature:
		...
		ping_data[peer_id].timer.connect(
			'timeout', self, '_on_ping_interval', [peer_id], CONNECT_ONESHOT
		)
