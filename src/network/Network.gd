...
# func create_server():
# 	# connect signals
# 	get_tree().connect("peer_connected",Callable(self,"peer_connected_to_server"))
# 	get_tree().connect("peer_disconnected",Callable(self,"peer_disconnected_from_server"))
...
# 		result = net.listen(server_info.port, PackedStringArray(), true)
# 		websocket = net
# 	else:
...
# 	Log("Server created, hosting checked port %s" % str(server_info.port))
	
# 	# update internal state
# 	net_id = get_tree().get_unique_id()
# 	connected = true
# 	is_server = true
...
# 		}
# 		ControlWebsocket.connect_to_server('/server', handshake)
# 		ControlWebsocket.connect('rx',Callable(self,'server_rx'))
# 		register_server_commands()
# 	else:
# 		Game.load_scene(default_scene)

# func server_rx(msg):
# 	Log('server_rx: ' + str(msg))
# 	if msg.type == 'load_scene':
# 		Game.load_scene(msg.scene)
...
# 	# TODO: this will need to be more transactional later checked
...
# 	}
# 	ControlWebsocket.connect_to_server('/client', handshake)
# 	ControlWebsocket.connect('rx',Callable(self,'client_rx'))

# func client_rx(msg):
# 	Log('client_rx: ' + str(msg))
# 	if msg.type == 'connect_to':
# 		connection_info.ip = msg.data.ip
# 		connection_info.port = msg.data.port
# 		_join_server()
...
# 		result = net.connect_to_url(url, PackedStringArray(), true)
# 		websocket = net
# 	else:
# 		net = ENetMultiplayerPeer.new()
...
# 	if !Game.returning_to_menu:
# 		Game.load_scene('mainmenu')

# # Client recieves server info from server
# @rpc("any_peer") func recieve_server_info(sinfo):
# 	if !is_server:
# 		server_info = sinfo
...
# # client only
# @rpc("any_peer") func player_registry_updated(registry):
...
# @rpc("any_peer") func kicked(reason):
...
# 		super.add_argument('scene', TYPE_STRING)\
# 		super.set_description('Change your multiplayer name.')\
# 		super.register()

# 	Console.add_command('set_address', self, 'set_address')\
# 		super.add_argument('address', TYPE_STRING)\
# 		super.set_description('Change the multiplayer server address.')\
# 		super.register()

# func register_server_commands():
# 	if is_server:
# 		Console.add_command('change_scene_to_file', self, 'change_scene_to_file')\
# 			super.add_argument('scene', TYPE_STRING)\
# 			super.set_description('Change the multiplayer scene.')\
# 			super.register()
