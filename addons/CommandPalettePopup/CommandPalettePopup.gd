...
func _unhandled_key_input(event: InputEventKey) -> void:
	if (
		event.as_text() == palette_settings.keyboard_shortcut_LineEdit.text
		and event.pressed
		and visible
		and not filter.text
		and filter.has_focus()
	):
...
func _switch_to_recent_file() -> void:
	if last_label.has_meta('Path'):
		if current_main_screen in ['2D', '3D', 'Script']:
			(
				_open_scene(last_label.get_meta('Path'))
				if scenes.has(last_label.get_meta('Path'))
				else _open_script(scripts[last_label.get_meta('Path')].ScriptResource)
			)
		else:
			(
				_open_scene(current_label.get_meta('Path'))
				if scenes.has(current_label.get_meta('Path'))
	...
	if selection:
		if current_filter == FILTER.SETTINGS:
			...
			if current_filter in [FILTER.ALL_OPEN_SCENES, FILTER.ALL_OPEN_SCRIPTS]:
				path = (
					item_list.get_item_text(selection[0] + 1)
					+ ('/' if not item_list.get_item_text(selection[0] + 1).ends_with('/') else '')
			...
			var path: String = ''
			path = (
				item_list.get_item_text(selected_index - 1) + selected_name
				if item_list.get_item_text(selected_index - 1).begins_with('./')
				else '.'
			)
			OS.clipboard = path
			...
			var path: String = filter.text.substr(
				palette_settings.keyword_folder_tree_LineEdit.text.length()
			)
...
func _on_AddButton_pressed() -> void:
	if filter.text.begins_with(palette_settings.keyword_editor_settings_LineEdit.text):
		...
		if selection:
			...
			hide()
			SCRIPT_CREATE_DIALOG.config(
				script_added_to.get_class(),
				(file_path if file_path else 'res:/') + '/' + selected_name + '.gd'
			)
		...
		if current_filter == FILTER.ALL_OPEN_SCRIPTS:
			...
			while selected_name != SCRIPT_LIST.get_item_text(SCRIPT_LIST.get_item_at_position(pos)):
				...
				if pos.y > OS.get_screen_size().y:
					push_warning(
						'Command Palette Plugin: Error getting context menu from script list.'
					)
					return
			pos.y += 5 * screen_factor
			hide()
			...
			for child in EDITOR.get_children():
				if child is PopupMenu:
					...
					child.call_deferred(
						'set_position',
						(
							SCRIPT_LIST.rect_global_position
							+ Vector2(SCRIPT_LIST.rect_size.x - 25, pos.y)
						)
					)
			...
			if not current_main_screen in ['2D', '3D']:
				(
					...
					if INTERFACE.get_edited_scene_root() is Spatial
					else INTERFACE.set_main_screen_editor('2D')
				)
				yield(get_tree(), 'idle_frame')
			...
			old_dock_tab_was_visible = (
				scene_tree_dock.get_parent().visible
				and scene_tree_dock.get_parent().get_parent().visible
			)
			var i = 0
			...
			sel.clear()
			var node_path = (
				item_list.get_item_text(selection[0] - 1) + selected_name
				if item_list.get_item_text(selection[0] - 1).begins_with('./')
				else '.'
			)
			...
			while selected_name != scene_tree.get_item_at_position(pos).get_text(0):
				...
				if pos.y > OS.get_screen_size().y:
					push_warning(
						'Command Palette Plugin: Error getting context menu from SceneTreeDock.'
					)
					return
			pos.y += 5 * screen_factor
			hide()
			...
			for child in scene_tree_dock.get_children():
				if child is PopupMenu:
					...
					child.call_deferred(
						'set_position',
						scene_tree.rect_global_position + Vector2(0, pos.y + 25 * screen_factor)
					)
					if not child.is_connected(
						'popup_hide', self, '_on_node_and_file_context_menu_hide'
					):
			...
			current_filter
			in [FILTER.ALL_FILES, FILTER.ALL_SCENES, FILTER.ALL_SCRIPTS, FILTER.TREE_FOLDER]
		):
			var path: String
			if current_filter == FILTER.TREE_FOLDER:
				path = filter.text.substr(
					palette_settings.keyword_folder_tree_LineEdit.text.length()
				)
				while path.begins_with('/') or path.begins_with(':'):
					path.erase(0, 1)
				...
				path = (
					item_list.get_item_text(selection[0] - 1)
					+ ('/' if not item_list.get_item_text(selection[0] - 1) == 'res://' else '')
			...
			old_dock_tab_was_visible = (
				filesystem_dock.get_parent().visible
				and filesystem_dock.get_parent().get_parent().visible
			)
			var i = 0
			...
			var pos = Vector2(30, 5) * screen_factor  # x = 30 so we don't click the folding arrow
			if (
				(file_split_view and current_filter != FILTER.TREE_FOLDER)
				or (
					current_filter == FILTER.TREE_FOLDER
					and not item_list.get_item_icon(selection[0])
				)
			):  # icon => folder
				while (
					path.get_file()
					!= file_list.get_item_text(file_list.get_item_at_position(pos))
				):
					pos.x = 5
					pos.y += 5
					if pos.y > OS.get_screen_size().y:
						push_warning(
							'Command Palette Plugin: Error getting context menu from FileSystemDock.'
						)
						return
				pos.y += 5 * screen_factor
				hide()
			...
			for child in filesystem_dock.get_children():
				if child is PopupMenu:
					...
					child.call_deferred(
						...
						(
							(
								file_list.rect_global_position
								if (
									(file_split_view and current_filter != FILTER.TREE_FOLDER)
									or (
										current_filter == FILTER.TREE_FOLDER
								...
								else file_tree.rect_global_position
							)
							+ Vector2(0, pos.y + 25 * screen_factor)
						)
					)
					if not child.is_connected(
						'popup_hide', self, '_on_node_and_file_context_menu_hide'
...
func _on_node_and_file_context_menu_hide() -> void:
	...
	if not old_dock_tab_was_visible:
		...
		if (
			old_dock_tab.get_parent().get_parent().get_parent()
			!= BASE_CONTROL_VBOX.get_child(1).get_child(1)
		):
...
func _on_SignalPopupMenu_index_pressed(index: int) -> void:
	...
	var selected_index = item_list.get_selected_items()[0]
	var node_path = (
		item_list.get_item_text(selected_index - 1) + item_list.get_item_text(selected_index)
		if item_list.get_item_text(selected_index - 1).begins_with('./')
		else '.'
	)
...
func _get_node_dock_tree_item(
	root: TreeItem, signal_name: String, connection_dock_tree: Tree
) -> void:
...
func _on_Filter_text_changed(new_txt: String) -> void:
	...
	if filter.text.ends_with('  '):
		...
		if selection:
			if (
				current_filter
				in [FILTER.ALL_FILES, FILTER.ALL_SCENES, FILTER.ALL_SCRIPTS, FILTER.SETTINGS]
			):
				var key = ''
				var keywords = [
					palette_settings.keyword_goto_line_LineEdit.text,
					palette_settings.keyword_goto_method_LineEdit.text,
					palette_settings.keyword_all_files_LineEdit.text,
					palette_settings.keyword_all_scenes_LineEdit.text,
					palette_settings.keyword_all_scripts_LineEdit.text,
					palette_settings.keyword_all_open_scenes_LineEdit.text,
					palette_settings.keyword_select_node_LineEdit.text,
				...
				sel.add_node(
					INTERFACE.get_edited_scene_root().get_node(
						node_path if node_path.begins_with('./') else '.'
					)
				)
				filter.text = ''
				filter.grab_focus()
				...
				var path = (
					(
						filter.text.substr(palette_settings.keyword_folder_tree_LineEdit.text.length()).strip_edges().rsplit(
							'/', true, 1
						)[0]
						+ '/'
					)
					if filter.text.count('/') > 0
					else '://'
				)
				filter.text = (
					palette_settings.keyword_folder_tree_LineEdit.text
					+ path
					+ item_list.get_item_text(selection[0])
				)
...
func _activate_item(selected_index: int=-1) -> void:
	...
	if (
		selected_index == -1
		or item_list.is_item_disabled(selected_index)
		or item_list.get_item_text(selected_index) == ''
		or item_list.get_item_custom_fg_color(selected_index) == secondary_color
		or selected_index % item_list.max_columns == 0
	):
		hide()
		return
	...
	if current_filter == FILTER.GOTO_METHOD:
		...
		if current_filter in [FILTER.ALL_OPEN_SCENES, FILTER.ALL_OPEN_SCRIPTS]:
			path = (
				item_list.get_item_text(selected_index + 1)
				+ ('/' if item_list.get_item_text(selected_index + 1) != 'res://' else '')
		...
		if (
			...
			and item_list.get_item_metadata(selected_index)
		):
			# open docs
			INTERFACE.set_main_screen_editor('Script')
			EDITOR.get_child(0).get_child(1).get_child(0).get_child(0).get_child(1).select(
				item_list.get_item_metadata(selected_index), true
			)
			EDITOR.get_child(0).get_child(1).get_child(0).get_child(0).get_child(1).emit_signal(
				'item_selected', item_list.get_item_metadata(selected_index)
			)
		else:
		...
		path = (
			'res://'
			+ (
				path.rsplit('/', true, 1)[0] + '/' + selected_name
				if path.count('/') > 0
				else selected_name
			)
		)
		...
		push_warning(
			'Command Palette Plugin: You should not be seeing this message. Please open an issue on Github and tell me what you did to see this.'
		)

	hide()

func _open_settings(setting_path: Array, setting_name: String, editor: bool=true) -> void:
	var popup: PopupMenu = BASE_CONTROL_VBOX.get_child(0).get_child(0).get_child(3 if editor else 1).get_child(
		0
	)
	...
	var SETTINGS_DIALOG = INTERFACE.get_base_control().get_child(
		INTERFACE.get_base_control().get_child_count() - 1
	)
	var SETTINGS_TREE: Tree = SETTINGS_DIALOG.get_child(3).get_child(0).get_child(1).get_child(0).get_child(
		0
	)
	var SETTINGS_INSPECTOR = SETTINGS_DIALOG.get_child(3).get_child(0).get_child(1).get_child(1).get_child(
		0
	)
...
func _inspector_property_editor_grab_focus(
	settings_name: String, node: Node=INTERFACE.get_inspector().get_child(0)
):  # Inpsector dock is default
	if node is EditorProperty:
		if node.get_edited_property() == settings_name:
			...
			push_warning(
				(
					'Command Palette Plugin: Problem grabbing focus of a property/setting. '
...
func _open_scene(path: String) -> void:
	...
	selection.add_node(INTERFACE.get_edited_scene_root())  # to see the "Node" dock in Script view
	(
		INTERFACE.call_deferred('set_main_screen_editor', '3D')
		if INTERFACE.get_edited_scene_root() is Spatial
...
func _update_popup_list(just_popupped: bool=false) -> void:
	if just_popupped:
		rect_size = Vector2(
			palette_settings.width_LineEdit.text as float,
			palette_settings.max_height_LineEdit.text as float
		)
		popup_centered()
		filter.grab_focus()
	...
	var qs_starts_at = (
		search_string.strip_edges().find_last(' ')
		if not search_string.begins_with(' ')
		else search_string.find_last(' ')
	)
	if (
		qs_starts_at != -1
		and not search_string.begins_with(palette_settings.keyword_goto_line_LineEdit.text)
	):
		...
		if not current_main_screen == 'Script':
			...
			if number.is_valid_integer():
				item_list.set_item_text(
					...
					'Go to line %s of %s.' % [clamp(number as int, 1, max_lines + 1), max_lines + 1]
				)
	...
	# select node
		...
		_build_node_list(
			INTERFACE.get_edited_scene_root(),
			search_string.substr(palette_settings.keyword_select_node_LineEdit.text.length()).strip_edges()
		)
		_count_node_list()
	...
	# file plugin
		...
		if FILELIST:
			...
			_build_item_list(
				search_string.substr(
					palette_settings.keyword_texteditor_plugin_LineEdit.text.length()
				)
			)
		else:
	...
	# todo plugin
		...
		if todo_dock:
			...
			_build_todo_list(
				search_string.substr(palette_settings.keyword_todo_plugin_LineEdit.text.length()),
				todo_dock.get_child(1).get_child(0)
			)
		else:
	...
	# edit editor settings
		...
		_build_item_list(
			search_string.substr(palette_settings.keyword_editor_settings_LineEdit.text.length())
		)
	...
	# edit inspector settings
		...
		_build_item_list(
			search_string.substr(palette_settings.keyword_set_inspector_LineEdit.text.length())
		)
	...
	# methods of the current script
		...
		if not current_main_screen == 'Script':
			...
			current_filter = FILTER.GOTO_METHOD
			_build_item_list(
				search_string.substr(palette_settings.keyword_goto_method_LineEdit.text.length())
			)
	...
	# show all scripts and scenes
		...
		_build_item_list(
			search_string.substr(palette_settings.keyword_all_files_LineEdit.text.length())
		)
	...
	# show all scripts
		...
		_build_item_list(
			search_string.substr(palette_settings.keyword_all_scripts_LineEdit.text.length())
		)
	...
	# show all scenes
		...
		_build_item_list(
			search_string.substr(palette_settings.keyword_all_scenes_LineEdit.text.length())
		)
	...
	# show open scenes
		...
		_build_item_list(
			search_string.substr(palette_settings.keyword_all_open_scenes_LineEdit.text.length())
		)
	...
	quickselect_line = clamp(
		quickselect_line as int, 0, item_list.get_item_count() / item_list.max_columns - 1
	)
	if item_list.get_item_count() >= item_list.max_columns:
		item_list.select(
			(
				quickselect_line * item_list.max_columns
				+ (
					1
					if (
						current_filter
...
func _build_help_page() -> void:
	...
	info_box.bbcode_text = (
		...
		% [
			...
			palette_settings.keyword_goto_method_LineEdit.text,
			palette_settings.keyword_set_inspector_LineEdit.text,
			palette_settings.keyword_texteditor_plugin_LineEdit.text,
			palette_settings.keyword_todo_plugin_LineEdit.text
		]
	)
	file.close()
...
func _build_item_list(search_string: String) -> void:
	...
	match current_filter:
		FILTER.ALL_FILES:
			for path in scenes:
				if (
					search_string
					and not path.matchn('*' + search_string + '*')
					and not search_string.is_subsequence_ofi(path)
				):
					continue
		...
		FILTER.ALL_OPEN_SCENES:
			...
			for path in open_scenes:
				if (
					search_string
					and not path.get_file().matchn('*' + search_string + '*')
					and not search_string.is_subsequence_ofi(path.get_file())
				):
					continue
				if search_string and search_string.is_subsequence_ofi(path.get_file()):
					name_matched_list.push_back(path)
		...
		FILTER.GOTO_METHOD:
			...
			if current_script:  # if not current_script => help page
				...
				for method in current_script.get_script_method_list():
					if (
						search_string
						and not search_string.is_subsequence_ofi(method.name)
						and not method.name.matchn('*' + search_string + '*')
					):
						continue
		...
		FILTER.FILEEDITOR:
			for idx in FILELIST.get_item_count():
				...
				if (
					search_string
					and not file.matchn('*' + search_string + '*')
					and not search_string.is_subsequence_ofi(file)
				):
					continue
		...
		FILTER.SETTINGS:
			for setting in editor_settings:
				if (
					search_string
					and not setting.matchn('*' + search_string + '*')
					and not search_string.is_subsequence_ofi(setting)
				):
					continue
			...
			if INTERFACE.get_selection().get_selected_nodes():
				...
				for property in node.get_property_list():
					if property.name and property.usage & PROPERTY_USAGE_EDITOR:
						if (
							search_string
							and not property.name.matchn('*' + search_string + '*')
							and not search_string.is_subsequence_ofi(property.name)
						):
							continue
		...
		if current_filter in [FILTER.ALL_FILES, FILTER.ALL_SCENES, FILTER.ALL_SCRIPTS]:
			...
			if scenes.has(name_matched_list[idx]):
				item_list.set_item_icon(
					item_list.get_item_count() - 1, scenes[name_matched_list[idx]].Icon
				)
			elif scripts.has(name_matched_list[idx]):
				item_list.set_item_icon(
					item_list.get_item_count() - 1, scripts[name_matched_list[idx]].Icon
				)
			elif other_files.has(name_matched_list[idx]):
				item_list.set_item_icon(
					item_list.get_item_count() - 1, other_files[name_matched_list[idx]].Icon
				)
		else:
			item_list.add_item(name_matched_list[idx].get_file())
			if scenes.has(name_matched_list[idx]):
	...
	for idx in path_matched_list.size():
		...
		if current_filter == FILTER.SETTINGS:
			item_list.add_item(
				'Editor :: ' if editor_settings.has(path_matched_list[idx]) else 'Project :: ',
				null,
				false
			)
			item_list.set_item_disabled(item_list.get_item_count() - 1, true)
			...
			if current_filter in [FILTER.ALL_FILES, FILTER.ALL_SCENES, FILTER.ALL_SCRIPTS]:
				...
				if scenes.has(path_matched_list[idx]):
					item_list.set_item_icon(
						item_list.get_item_count() - 1, scenes[path_matched_list[idx]].Icon
					)
				elif scripts.has(path_matched_list[idx]):
					item_list.set_item_icon(
						item_list.get_item_count() - 1, scripts[path_matched_list[idx]].Icon
					)
				elif other_files.has(path_matched_list[idx]):
					item_list.set_item_icon(
						item_list.get_item_count() - 1, other_files[path_matched_list[idx]].Icon
					)
			else:
				item_list.add_item(path_matched_list[idx].get_file())
				if scenes.has(path_matched_list[idx]):
	...
	if (
		palette_settings.include_help_pages_button.pressed
		and current_filter == FILTER.ALL_OPEN_SCRIPTS
	):
		for script_index in (
			EDITOR.get_child(0).get_child(1).get_child(0).get_child(0).get_child(1).get_item_count()
			- EDITOR.get_open_scripts().size()
		):
			if (
				EDITOR.get_child(0).get_child(1).get_child(0).get_child(0).get_child(1).get_item_text(script_index + EDITOR.get_open_scripts().size()).matchn(
					'*' + search_string + '*'
				)
				or search_string.is_subsequence_ofi(
					EDITOR.get_child(0).get_child(1).get_child(0).get_child(0).get_child(1).get_item_text(
						script_index + EDITOR.get_open_scripts().size()
					)
				)
			):
				item_list.add_item(' ' + String(index) + '  :: ', null, false)
				...
				item_list.set_item_metadata(
					item_list.get_item_count() - 1, script_index + EDITOR.get_open_scripts().size()
				)
				item_list.add_item('')
				index += 1
...
func _build_todo_list(search_string: String, todo_dock_tree: Control) -> void:
	...
	while file:
		...
		while todo:
			...
			if (
				not search_string
				or todoname.matchn('*' + search_string.strip_edges().replace(' ', '*') + '*')
				or search_string.is_subsequence_ofi(todoname)
			):
				item_list.add_item(' ' + String(counter) + '  :: ', null, false)
...
func _build_node_list(root: Node, search_string: String) -> void:
	if (
		not search_string
		or root.name.matchn('*' + search_string.strip_edges().replace(' ', '*') + '*')
		or search_string.is_subsequence_ofi(root.name)
	):
		...
		if root == INTERFACE.get_edited_scene_root():
			...
			item_list.add_item(
				(
					'./'
					+ String(INTERFACE.get_edited_scene_root().get_path_to(root.get_parent()))
					+ '/'
				),
				null,
				false
...
func _build_folder_view(search_string: String) -> void:
	...
	for folder_path in folders:
		var fname = search_string.substr(
			search_string.get_base_dir().length() + (1 if search_string.count('/') > 0 else 0)
		)
		if (
			(
				('res://' + search_string.get_base_dir() + ('/' if search_string.count('/') != 0 else '')).to_lower()
				== folders[folder_path].Parent_Path.to_lower()
			)
			...
			if folders[folder_path].Subdir_Count:
				item_list.add_item(
					(
						' Subdirs: '
						+ String(folders[folder_path].Subdir_Count)
						+ (
							' + Files: %s' % folders[folder_path].File_Count
							if folders[folder_path].File_Count
							else ''
						)
					),
					null,
	...
	for path in scenes:
		if (
			('res://' + search_string.get_base_dir().to_lower() != path.get_base_dir().to_lower())
			or not path.get_file().matchn(search_string.get_file() + '*')
		):
			continue
		list.push_back(path)
...
func _adapt_list_height() -> void:
	if palette_settings.adaptive_height_button.pressed:
		...
		var margin = (
			filter.rect_size.y
			+ $PaletteMarginContainer.margin_top
			+ abs($PaletteMarginContainer.margin_bottom)
			+ $PaletteMarginContainer/VBoxContainer/MarginContainer.get(
				'custom_constants/margin_top'
			)
			+ $PaletteMarginContainer/VBoxContainer/MarginContainer.get(
				'custom_constants/margin_bottom'
			)
...
func _update_files_dictionary(folder: EditorFileSystemDirectory, reset: bool=false) -> void:
	...
	for subdir in folder.get_subdir_count():
		folders[folder.get_subdir(subdir).get_path()] = {
			'Subdir_Count': folder.get_subdir(subdir).get_subdir_count(),
			'File_Count': folder.get_subdir(subdir).get_file_count(),
			'Folder_Name': folder.get_subdir(subdir).get_name(),
...
func _update_recent_files():
	...
	if not recent_files_are_updating and current_main_screen in ['2D', '3D', 'Script']:
		...
		if current_main_screen == 'Script':
			if (
				EDITOR.get_child(0).get_child(1).get_child(0).get_child(0).get_child(1).get_selected_items()
				and (
					EDITOR.get_child(0).get_child(1).get_child(0).get_child(0).get_child(1).get_selected_items()[0]
					>= EDITOR.get_open_scripts().size()
				)
			):
				if palette_settings.include_help_pages_button.pressed:
					current_label.text = SCRIPT_LIST.get_item_text(
						SCRIPT_LIST.get_selected_items()[0]
					)
				...
				current_label.text = (
					path
					if palette_settings.show_path_for_recent_button.pressed
					else path.get_file()
				)
...
func _update_editor_settings() -> void:  # only called during startup because editor settings can't be changed via normal means
	for setting in EDITOR_SETTINGS.get_property_list():
		...
		if (
			setting.name
			and setting.name.find('/') != -1
			and setting.usage & PROPERTY_USAGE_EDITOR
			and not setting.name.begins_with('favorite_projects/')
		):
