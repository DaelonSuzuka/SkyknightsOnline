...
func _predict_path() -> void:
	...
	for _path in settings:
		if _path.begins_with(preds[preds.size() - 1]):
			...
			prediction_list.rect_global_position = Vector2(
				path.rect_global_position.x + path.text.length() * 8,
				path.rect_global_position.y + path.rect_size.y
			)
			prediction_list.set_deferred(
				'rect_size',
				Vector2(max_length * 10, prediction_list.get_item_count() * 15) * screen_factor
			)
...
func _cast_value(text: String, type: int):
	match type:
		...
		TYPE_DICTIONARY:
			...
			for pair in a:
				var tmp = pair.split(':')
				result[tmp[0] as float if tmp[0].is_valid_float() else tmp[0]] = (
					tmp[1] as float
					if tmp[1].is_valid_float()
					else tmp[1]
				)
			return result
