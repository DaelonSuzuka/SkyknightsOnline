...
func load_settings() -> void:
	...
	if error == ERR_FILE_NOT_FOUND:
		...
		keyword_all_open_scenes_LineEdit.text = config.get_value(
			'Settings', 'keyword_all_open_scenes'
		)
		keyword_select_node_LineEdit.text = config.get_value('Settings', 'keyword_select_node')
		keyword_editor_settings_LineEdit.text = config.get_value(
			'Settings', 'keyword_editor_settings'
		)
		...
		keyword_texteditor_plugin_LineEdit.text = config.get_value(
			'Settings', 'keyword_texteditor_plugin'
		)
		...
		show_path_for_recent_button.pressed = bool(
			config.get_value('Settings', 'show_path_for_recent')
		)
...
func save_settings() -> void:
	...
	config.set_value(
		'Settings', 'keyword_texteditor_plugin', keyword_texteditor_plugin_LineEdit.text
	)
	config.set_value('Settings', 'keyword_todo_plugin', keyword_todo_plugin_LineEdit.text)
	config.set_value(
		'Settings',
		'hide_script_panel',
		hide_script_panel_button.pressed if hide_script_panel_button.pressed else ''
	)
	config.set_value(
		'Settings',
		'include_help_pages',
		include_help_pages_button.pressed if include_help_pages_button.pressed else ''
	)
	config.set_value(
		'Settings',
		'adaptive_height',
		adaptive_height_button.pressed if adaptive_height_button.pressed else ''
	)
	config.set_value(
		'Settings',
		'show_path_for_recent',
		show_path_for_recent_button.pressed if show_path_for_recent_button.pressed else ''
	)
